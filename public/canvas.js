  ...
  if (window.addEventListener) {
    window.addEventListener('load', function () {
      ...
      function init() {
        ...
        canvas.addEventListener('mousemove', (ev) => ev.type && throttle(ev_canvas(ev), 10), false);
      ...
      function disableWhiteboard(action) {
        const locaItem = localStorage.getItem("classroom")
        let isTeacher = false
        if(locaItem) {
          const classroom = JSON.parse(locaItem)
      ...
      $("#disable-checkbox").on('change', function () {
        const isChecked = this.checked
        socket.emit('Disableboard', isChecked);
      });

      socket.on('DisableboardAction', disableWhiteboard);
